...
fn dial_target(host: &str, port: u16, addr: &SocketAddr) -> Option<f64> {
    // While the TUN captures all traffic, direct dials are answered locally
    // in ~1ms; go through the tunnel's SOCKS proxy for a real measurement.
    if crate::ops::local_proxy_up() {
        crate::ops::socks_endpoint_rtt(host, port).map(|ms| ms as f64)
    } else {
        dial(addr)
    }
}
...
pub fn measure(target: &DeepTarget) -> DeepSample {
    ...
    for slot in 0..ROUNDS {
        match dial_target(&target.host, target.port, &addr) {
