...
impl ConfigGenerator {
    ...
    pub fn with_privacy(
        ...
        let (a, b) = match bootstrap {
            ...
            "digitale" => ("185.95.218.42", "185.95.218.43"),
    ...
    fn build_dns(&self) -> Value {
        ...
        let (remote_a, remote_b, doh_port, tls_port): (&str, &str, u16, u16) =
            match self.dns_remote.as_str() {
                ...
                "digitale" => ("185.95.218.42", "185.95.218.43", 443, 853),
        ...
        if let Some(custom) = &self.dns_custom_doh {
                ...
                .filter(|h| !h.is_empty())
            {
                ...
                if host.parse::<std::net::IpAddr>().is_err() {
                    // The custom DoH endpoint is a hostname: it must resolve
                    // through the IP-based bootstrap server, not itself.
                    remote_doh["domain_resolver"] = json!("bootstrap-dns");
                }
            }
        }
    ...
    fn build_tls_plain(&self, sni: &str, fp: &str, insecure: bool) -> Value {
        if insecure {
            log::warn!(
                "TLS certificate validation bypass requested by subscription for sni={}",
                sni
            );
        }
    ...
    fn build_vmess(&self, p: &VmessParams) -> Result<Value, VpnError> {
        ...
        if p.tls {
            out["tls"] = self.build_tls_plain(&p.sni, "chrome", p.insecure);
        }
    ...
    fn build_hysteria2(&self, p: &Hysteria2Params) -> Value {
        if p.insecure {
            log::warn!("subscription requested TLS bypass for hysteria2 (sni={})", p.sni);
        }
    ...
    fn build_tuic(&self, p: &TuicParams) -> Value {
        if p.insecure {
            log::warn!("subscription requested TLS bypass for tuic (sni={})", p.sni);
        }
