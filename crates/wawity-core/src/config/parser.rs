...
fn parse_insecure_flag(q: &HashMap<String, String>) -> bool {
    ["allowInsecure", "allow_insecure", "insecure"]
        .iter()
        .any(|k| q.get(*k).map(|v| v == "1" || v == "true").unwrap_or(false))
}
...
fn parse_vless(raw: &str) -> Result<ProxyConfig, VpnError> {
    ...
    let insecure = parse_insecure_flag(&q);
...
fn parse_vmess(raw: &str) -> Result<ProxyConfig, VpnError> {
    ...
    let header_type = v.header_type.clone().unwrap_or_default();
    let insecure = v
        .allow_insecure
        .as_ref()
        .map(|val| match val {
...
mod tests {
    ...
    #[test]
    fn insecure_flags_accepted() {
        let url = "vless://uuid@1.2.3.4:443?security=tls&insecure=1#A";
        match parse_vless(url).unwrap() {
            ProxyConfig::Vless(p) => assert!(p.insecure),
            _ => panic!("wrong variant"),
        }
        let url = "trojan://pw@1.2.3.4:443?allow_insecure=true&sni=s.com&serviceName=grpc-svc#B";
        match parse_trojan(url).unwrap() {
            ProxyConfig::Trojan(p) => {
                assert!(p.insecure);
                assert_eq!(p.service_name, "grpc-svc");
            }
            _ => panic!("wrong variant"),
        }
        let url = "hysteria2://pw@1.2.3.4:8443?allowInsecure=1#C";
        match parse_hysteria2(url).unwrap() {
            ProxyConfig::Hysteria2(p) => assert!(p.insecure),
            _ => panic!("wrong variant"),
        }
    }

    #[test]
    fn vmess_insecure_and_header_type() {
        let blob = serde_json::json!({
            "id": "uuid-1", "add": "srv.com", "port": "443", "tls": "tls",
            "allowInsecure": true, "type": "http", "net": "tcp",
            "path": "/h", "host": "srv.com"
        });
        let raw = format!("vmess://{}", general_purpose::STANDARD.encode(blob.to_string()));
        match parse_vmess(&raw).unwrap() {
            ProxyConfig::Vmess(p) => {
                assert!(p.insecure);
                assert_eq!(p.header_type, "http");
                assert!(p.tls);
            }
            _ => panic!("wrong variant"),
        }
