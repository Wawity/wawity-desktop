...
/// IPv4 address of the best physical (non-virtual) adapter — the one the OS
/// would use for the default route. Sockets bound to this source address
/// egress outside the TUN even while the tunnel is up (Windows strong-host
/// send model), which is exactly how split-tunneled traffic escapes.
pub fn physical_interface_ipv4() -> Option<std::net::Ipv4Addr> {
    let alias = default_physical_interface()?;
    ...
    unsafe {
        for _ in 0..3 {
            ...
            while !node.is_null() {
                ...
                if adapter.OperStatus == IfOperStatusUp && name.eq_ignore_ascii_case(&alias) {
                    let mut uni = adapter.FirstUnicastAddress;
                    while !uni.is_null() {
                        let sockaddr = (*uni).Address.lpSockaddr;
