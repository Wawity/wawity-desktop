...
fn wrap(context: &str, err: windows::core::Error) -> VpnError {
    ...
    // 0x800706D9 (ERROR_NO_MORE_ENDPOINTS_MAPPER): the Windows Firewall
    // service (MpsSvc) is stopped or disabled on this machine.
    if (err.code().0 & 0xFFFF) == 0x06D9 {
        msg.push_str(
            " — Windows Firewall service looks disabled; start 'Windows Defender Firewall' in services.msc and try again",
        );
    }
    VpnError::NetworkError(msg)
}
