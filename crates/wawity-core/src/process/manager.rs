...
impl ProcessManager {
    ...
    fn find_singbox_binary() -> Result<std::path::PathBuf, VpnError> {
        ...
        if let Ok(env_path) = std::env::var("WAWITY_SINGBOX") {
        ...
        let candidates = vec![
            ...
            exe_dir.parent().map(|p| p.join("sing-box-x86_64.exe")).unwrap_or_default(),
    ...
    pub fn stop(&self) -> Result<(), VpnError> {
        self.stop_within(Duration::from_millis(1500))
    }

    pub fn stop_fast(&self) -> Result<(), VpnError> {
        self.stop_within(Duration::from_millis(200))
    }

    pub fn stop_within(&self, grace: Duration) -> Result<(), VpnError> {
        let mut child_lock = self.child.lock().unwrap();
...
impl ProcessManager {
    pub fn detach(&self) -> Option<u32> {
        let mut child_lock = self.child.lock().unwrap();
        child_lock.take().map(|child| child.id())
    }
}
