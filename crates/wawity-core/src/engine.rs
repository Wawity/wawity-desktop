...
pub fn probe_socks(target: Option<&str>) -> Result<u64, String> {
    ...
    let host: String = match target {
        Some(h) if !h.trim().is_empty() => h.trim().to_string(),
        _ => "cp.cloudflare.com".to_string(),
    };
    let v4 = host.parse::<std::net::Ipv4Addr>().ok();
    let v6 = if v4.is_none() { host.parse::<std::net::Ipv6Addr>().ok() } else { None };
    ...
    if head[0] != 0x05 || head[1] != 0x00 {
        return Err(format!("socks-refused: dial failed through tunnel, reply code {:#04x}", head[1]));
    }
    ...
    stream
        .write_all(request)
        .map_err(|e| format!("blackhole: tunnel accepted dial but dropped request data: {}", e))?;
    drain_http_head(&mut stream, &mut scratch)
        .map_err(|e| format!("blackhole: tunnel accepted dial but no data came back: {}", e))?;
    ...
    if best == u128::MAX {
        return Err("blackhole: tunnel probe got no keep-alive response".to_string());
    }
