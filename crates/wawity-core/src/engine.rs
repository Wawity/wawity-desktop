...
pub fn switch_session(
    ...
    if is_locked_down {
        ...
        if let Err(e) = routing.stage_exceptions(&binary_path, &app_exe_path) {
            return Err(format!(
                "Switch leak guard setup failed, keeping current session: {}",
                e
            ));
        }
        ...
        if let Err(e) = routing.commit_connection() {
            ...
            return Err(format!(
                "Switch leak guard lock failed, keeping current session: {}",
                e
            ));
        }
        drop(routing);
...
pub fn reload_bypass_apps(
    ...
    if !is_locked_down {
        ...
        if let Err(e) = routing.stage_exceptions(&binary_path, &app_exe_path) {
            return Err(format!(
                "Leak guard setup failed, keeping current session: {}",
                e
            ));
        }
        if let Err(e) = routing.allow_server_endpoint(&direct_host, &privacy.bootstrap_dns) {
        ...
        if let Err(e) = routing.commit_connection() {
            routing.abort_staged_connection(false);
            return Err(format!(
                "Leak guard lock failed, keeping current session: {}",
                e
            ));
        }
    }
