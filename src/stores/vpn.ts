...
export const useVpnStore = defineStore('vpn', {
  state: () => ({
    ...
    tunnelBlocked: false,
    _blackholeStreak: 0,
    _blackholeFailingOver: false,
    _blackholeTriedIds: [] as string[],
    _blackholeLastAt: 0,
  ...
  actions: {
    ...
    startPolling() {
      ...
      // Fast poll only while connected; idle app settles for a slow tick.
      const interval = () =>
        this._pollHidden
          ? HIDDEN_POLL_MS
    ...
    _startSessionTimer() {
      ...
      this.tunnelBlocked = false;
      this._blackholeStreak = 0;
      this._blackholeTriedIds = [];
    ...
    async measureCurrentPing() {
      ...
      // Grace period: banners must not appear before the session is 5s old —
      // early probes hit a tunnel that is still settling.
      if (this.sessionSeconds < 5) return;
      ...
      try {
        ...
        const msg = String(e ?? '');
        const transportDead =
          msg.includes('blackhole') ||
          msg.includes('socks-refused') ||
          msg.includes('proxy closed');
        if (transportDead && this.sessionSeconds > 10) {
          this._blackholeStreak += 1;
          if (this._blackholeStreak >= 2) {
            this.tunnelBlocked = true;
            void this._blackholeFailover();
          }
        }
      }
    },
    ...
    /**
     * tunnelBlocked = connected but no data passes (RKN blackhole / server
     * blocked). Silently hop to another server of the same subscription and
     * verify the new tunnel with the DNS-free IP probe — a plain TLS
     * handshake can't tell a blocked server from a live one.
     */
    async _blackholeFailover() {
      if (!this.settings.auto_failover) return;
      if (this._blackholeFailingOver || this.loading || this.smartPicking || this.dnsCalibrating) return;
      if (this._disconnectIntent || !this.status.connected) return;
      if (Date.now() - this._blackholeLastAt < 10 * 60 * 1000) return;
      ...
      const pool = (subId
        ...
        (s) =>
          s.id !== this.selectedServerId &&
          !this._blackholeTriedIds.includes(s.id) &&
          !this.isServerExpired(s.id),
      );
      this._blackholeLastAt = Date.now();
      const { pushToast } = useNotifications();
      if (pool.length === 0) {
        pushToast('warning', t('calib.blockedAllTitle'), t('calib.blockedAllDesc'), 9000);
        return;
      }
      this._blackholeFailingOver = true;
      try {
        let ranked = [...pool];
        try {
          const samples = await this.deepProbe(
            pool.slice(0, 12).map((s) => ({ id: s.id, server: s.server })),
          );
          const byId = new Map(samples.map((r) => [r.id, r]));
          ranked = [...pool].sort((a, b) => {
            const va = byId.get(a.id);
            const vb = byId.get(b.id);
            const sa = va && va.reachable ? va.score : -1;
            const sb = vb && vb.reachable ? vb.score : -1;
            return sb - sa;
          });
        } catch {}
        for (const cand of ranked.slice(0, 3)) {
          if (!this.status.connected || this._disconnectIntent || this.dnsCalibrating) return;
          try {
            await this.switchServer(cand.id);
          } catch {
            this._blackholeTriedIds.push(cand.id);
            continue;
          }
          if (this.selectedServerId !== cand.id) {
            this._blackholeTriedIds.push(cand.id);
            continue;
          }
          await new Promise((r) => setTimeout(r, 2500));
          if (await this._probeTransportByIp(1)) {
            this.tunnelBlocked = false;
            this._blackholeStreak = 0;
            this._blackholeTriedIds = [];
            this.currentPingMs = null;
            await this.measureCurrentPing();
            pushToast(
              'success',
              t('calib.blockedSwitchedTitle'),
              t('calib.blockedSwitchedDesc', { name: cand.name }),
              7000,
            );
            return;
          }
        ...
        this._blackholeFailingOver = false;
      }
    },
    ...
    async _startDnsCalibration(auto: boolean) {
      ...
      if (auto) {
        ...
        const { pushToast } = useNotifications();
        try {
          // Transport check first: if the pipe itself is dead (RKN blackhole),
          // swapping resolvers is pointless — surface the blocked state instead.
          if (!(await this._probeTransportByIp(2))) {
            this.tunnelBlocked = true;
            this.dnsCalibrated = false;
            void this._blackholeFailover();
            return;
          }
          this.tunnelBlocked = false;
    ...
    /**
     * Probe the tunnel transport WITHOUT DNS: IP literals bypass the tunnel
     * resolver entirely. If these are dead, the server/pipe is blocked and no
     * DNS recalibration can help — that is the RKN blackhole signature.
     */
    async _probeTransportByIp(attempts: number): Promise<boolean> {
      const ips = ['1.1.1.1', '1.0.0.1', '9.9.9.9'];
      for (let i = 0; i < attempts; i++) {
        let alive = 0;
        for (const ip of ips) {
          try {
            const ms = await invoke<number | null>('measure_tunnel_latency', { host: ip });
            if (ms !== null && ms !== undefined && ms > 0) alive += 1;
          } catch {}
        }
        if (alive >= 2) return true;
    ...
    async refreshStatus() {
      try {
        ...
        this.syncDiscordPresence();
        if (wasConnected !== raw.connected && this._statusTimer) {
          this.stopPolling();
          this.startPolling();
        }
