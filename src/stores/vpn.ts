...
const DEFAULT_SETTINGS: AppSettings = {
  ...
  connection_servers: 'off' as 'off' | 'inline' | 'sidebar',
  protocol: 'auto',
...
export const useVpnStore = defineStore('vpn', {
  ...
  actions: {
    ...
    async _blackholeFailover() {
      ...
      if (this._blackholeFailingOver || this.loading || this.smartPicking) return;
      ...
      try {
        ...
        for (const cand of ranked.slice(0, 3)) {
          if (!this.status.connected || this._disconnectIntent) return;
          try {
    ...
    async refreshAllSubscriptions(): Promise<{
      ...
      let failed = 0;
      for (const sub of [...this.subscriptions]) {
        const r = await this.refreshSubscription(sub.id);
        if (r.error) failed += 1;
        else {
          added += r.added;
          removed += r.removed;
