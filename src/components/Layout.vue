  ...
  <div class="shell">
        ...
        <nav class="nav" :aria-label="t('layout.mainNav')">
          ...
          <router-link
            ...
            :class="['nav-link', {
              ...
              'nav-link--collapsed': item.to === '/servers' && serversNavHidden,
            }]"
    ...
    <nav v-if="!isStandalone" class="mobile-nav" :aria-label="t('layout.mobileNav')">
      <router-link
        ...
        :to="item.to"
        :class="['mobile-link', {
          'mobile-link--active': route.path === item.to,
          'mobile-link--collapsed': item.to === '/servers' && serversNavHidden,
        }]"
        :aria-current="route.path === item.to ? 'page' : undefined"
      >
...
const spawned = reactive({ home: false, settings: false, analysis: false, servers: false });
...
const SEARCH_ITEMS = [
  ...
  { labelKey: 'settings.connectionServers', tab: 'appearance', descKey: 'settings.connectionServersDesc', icon: Globe2, kindKey: 'layout.searchKindSetting', keywords: 'connection servers inline dropdown panel sidebar home embed' },
...
const serversNavHidden = computed(() => vpnStore.settings.connection_servers === 'sidebar');

const pillStyle = computed(() => {
  let idx = activeIndex.value;
  // Items after the collapsed "Servers" link shift one row up (36px height + 4px gap).
  if (serversNavHidden.value && idx > 2) idx -= 1;
  return {
    transform: `translateY(${Math.max(idx, 0) * 40}px)`,
...
.nav-link{position:relative;z-index:1;display:flex;align-items:center;gap:10px;height:36px;max-height:36px;padding:0 10px;border-radius:8px;font-size:13px;font-weight:500;color:var(--muted-foreground);text-decoration:none;transition:color .15s,background .15s,max-height .32s cubic-bezier(.22,1,.36,1),opacity .26s ease,margin-top .32s cubic-bezier(.22,1,.36,1)}

.nav-link--collapsed{max-height:0;opacity:0;pointer-events:none;overflow:hidden;margin-top:-4px}
...
@media(max-width:768px){.mobile-link--collapsed{display:none}}
