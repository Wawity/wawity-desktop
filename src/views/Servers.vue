<template>
  <div ref="pageRoot" class="page">
    ...
    <Transition name="fav-reveal">
      <div v-if="vpnStore.favoriteServers.length > 0" class="fav-strip">
        <div class="fav-head">
    ...
    <template v-else>
      <div class="subs-strip">
        <div v-for="sub in vpnStore.subscriptions" :key="sub.id" class="sub-chip">
          <button
            type="button"
            class="sub-badge"
      ...
      <ServerGlobe v-if="vpnStore.settings.server_view === 'globe'" :query="query" />
  ...
  <div class="page">
    ...
    <Transition name="preview-slide">
      <div v-if="preview" class="preview-card">
        ...
        <button class="add-sub-btn" type="button" @click="openConfirm" v-text="t('servers.addSubscription')" />
      </div>
    </Transition>
    ...
    <Transition name="fade">
      <div v-if="confirmOpen" class="confirm-overlay" @click.self="confirmOpen = false">
        <div class="confirm-modal">
          ...
          <p class="confirm-desc" v-text="addSubDescText" />
          <div class="confirm-actions">
            <button class="confirm-cancel" type="button" @click="confirmOpen = false" v-text="t('servers.cancel')" />
            <button class="confirm-ok" type="button" @click="addSubscription" v-text="t('servers.add')" />
          </div>
        </div>
      </div>
    </Transition>
    ...
    <Transition name="fade">
      <div v-if="deleteTarget" class="confirm-overlay" @click.self="deleteTarget = null">
        <div class="confirm-modal">
          ...
          <p class="confirm-desc" v-text="removeSubDescText" />
          <div class="confirm-actions">
            <button class="confirm-cancel" type="button" @click="deleteTarget = null" v-text="t('servers.cancel')" />
            <button class="confirm-ok confirm-ok--danger" type="button" @click="removeSubscription" v-text="t('servers.remove')" />
          </div>
        </div>
      </div>
    </Transition>
    ...
    <div v-if="vpnStore.subscriptions.length > 0" class="toolbar">
      <div class="search-bar">
        <Search :size="14" class="search-icon" />
        <input v-model="query" type="text" :placeholder="t('servers.searchServers')" class="search-input" />
      </div>
      <button
        class="tool-btn"
        type="button"
    ...
    <template v-else>
      <div class="subs-strip">
        <div v-for="sub in vpnStore.subscriptions" :key="sub.id" class="sub-chip">
          <span class="sub-chip-name" v-text="sub.name" />
          <span class="sub-chip-count mono" v-text="sub.servers.length" />
          <span v-if="expiryLabel(sub)" class="expiry-badge" :class="expiryClass(sub)" v-text="expiryLabel(sub)" />
          <span v-if="trafficLabel(sub)" class="traffic-badge mono" v-text="trafficLabel(sub)" />
          <button
            class="chip-btn"
            type="button"
...
import { ref, computed, onMounted, onUnmounted } from 'vue';
import {
  Clipboard, Search, Globe, Loader2, X,
  ShieldCheck, Trash2, Zap, Radio, Shuffle, RotateCw,
...
const totalServers = computed(() => vpnStore.subscriptions.reduce((a, s) => a + s.servers.length, 0));
const anyRefreshing = computed(() => vpnStore.refreshingSubIds.size > 0);
...
const removeSubDescText = computed(() =>
  deleteTarget.value
    ? t('servers.removeSubscriptionDesc', { name: deleteTarget.value.name, count: deleteTarget.value.count })
    : '',
);
...
async function refreshAllSubscriptions() {
  ...
  if (failCount > 0) {
    pushToast('warning', t('servers.refreshCompletedErrors'), t('servers.refreshCompletedErrorsDesc', { count: failCount }), 5000);
  } else if (totalAdded === 0 && totalRemoved === 0) {
    pushToast('info', t('servers.allUpToDate'), t('servers.allUpToDateDesc'));
  } else {
    const summary = `${t('servers.newCount', { count: totalAdded })}, ${t('servers.removedCount', { count: totalRemoved })}`;
...
async function autoSelect() { await vpnStore.autoSelectFastest(); }
async function pingAll() { await vpnStore.measureLatencies(); }
</script>
...
.page-sub { font-size: 12px; color: rgba(235, 238, 250, 0.55); }
...
.hop-banner .inline-icon { display: inline; vertical-align: -1px; }
...
.url-input:focus { border-color: rgba(167, 139, 250, 0.55); background: rgba(255, 255, 255, 0.07); }
.url-input:disabled { opacity: 0.5; }
.url-input::placeholder { color: rgba(235, 238, 250, 0.35); }
...
.ghost-btn:hover { color: #fff; background: rgba(255, 255, 255, 0.1); }
...
.fetch-btn {
  ...
  cursor: pointer;
  flex-shrink: 0;
  box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.18), 0 6px 18px rgba(124, 92, 255, 0.25);
  transition: opacity 160ms, transform 160ms, box-shadow 160ms;
}

.fetch-btn:hover:not(:disabled) { box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.22), 0 8px 24px rgba(124, 92, 255, 0.35); }
.fetch-btn:active:not(:disabled) { transform: scale(0.96); }
.fetch-btn:disabled { opacity: 0.4; cursor: not-allowed; }
.fetch-error { font-size: 12px; color: #ff8a92; padding: 0 4px; }
...
.preview-count { font-size: 11px; color: rgba(235, 238, 250, 0.5); }

.preview-list { list-style: none; max-height: 280px; overflow-y: auto; padding: 6px 0; }
...
.preview-info { display: flex; flex-direction: column; gap: 2px; min-width: 0; flex: 1; }
.preview-srv-name { font-size: 12.5px; font-weight: 500; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.preview-proto { font-size: 10.5px; color: rgba(235, 238, 250, 0.4); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.add-sub-btn:active { transform: scale(0.98); }
...
.confirm-desc { font-size: 12.5px; line-height: 1.5; color: rgba(235, 238, 250, 0.6); }
...
.search-icon { color: rgba(235, 238, 250, 0.4); flex-shrink: 0; }
...
.search-input::placeholder { color: rgba(235, 238, 250, 0.35); }
...
.tool-btn:hover:not(:disabled) { background: rgba(255, 255, 255, 0.1); color: #fff; }
...
.tool-btn--accent:hover:not(:disabled) { background: rgba(167, 139, 250, 0.22); color: #e6dcff; }
...
.empty-icon { color: rgba(235, 238, 250, 0.3); margin-bottom: 4px; }
.empty-sub { font-size: 11.5px; color: rgba(235, 238, 250, 0.4); }

.subs-strip { display: flex; flex-wrap: wrap; gap: 6px; }
...
.sub-chip-name { font-weight: 600; max-width: 180px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.sub-chip-count { font-size: 10.5px; color: rgba(235, 238, 250, 0.45); }
...
.expiry-badge--ok { background: rgba(94, 230, 154, 0.12); color: #5ee69a; }
.expiry-badge--warning { background: rgba(240, 211, 106, 0.12); color: #f0d36a; }
.expiry-badge--critical { background: rgba(255, 159, 107, 0.14); color: #ff9f6b; }
.expiry-badge--expired { background: rgba(255, 138, 146, 0.14); color: #ff8a92; }

.traffic-badge { font-size: 10px; color: rgba(235, 238, 250, 0.45); }
...
.chip-btn:hover:not(:disabled) { background: rgba(255, 255, 255, 0.12); color: #fff; }
.chip-btn:active:not(:disabled) { transform: scale(0.9); }
.chip-btn:disabled { opacity: 0.45; cursor: not-allowed; }

.chip-btn--danger:hover { background: rgba(255, 138, 146, 0.16); color: #ff8a92; }

.preview-slide-enter-active { transition: all 260ms cubic-bezier(0.34, 1.4, 0.64, 1); }
.preview-slide-leave-active { transition: all 160ms ease; }
.preview-slide-enter-from, .preview-slide-leave-to { opacity: 0; transform: translateY(-10px) scale(0.98); }
