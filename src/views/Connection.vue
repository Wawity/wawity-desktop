<template>
  <div ref="pageRef" class="page" :class="{ 'page--split': isSidebarMode }">
    <div class="conn-col">
    <div class="wordmark">
    ...
    <div class="server-card" data-sensitive>
      <button
        v-if="isInlineMode"
        type="button"
        class="server-main server-main--btn"
      ...
      >
        ...
        <ChevronRight :size="15" class="chevron" :class="{ 'chevron--open': showServerList }" aria-hidden="true" />
      </button>
      <div v-else-if="isSidebarMode" class="server-main server-main--plain">
        <div class="pill-left">
          <CountryFlag
            :code="vpnStore.selectedServer?.countryCode ?? 'UN'"
            :width="30"
      ...
      <router-link v-else to="/servers" class="server-main">
        <div class="pill-left">
          <CountryFlag
            :code="vpnStore.selectedServer?.countryCode ?? 'UN'"
            :width="30"
    ...
    <Transition name="server-list-reveal">
      <div v-if="isInlineMode && showServerList" class="server-list-wrap" data-sensitive>
        <ServerColumns query="" />
      </div>
    </Transition>
    ...
    <!-- Transport dead (RKN blackhole / server blocked) -->
    <Transition name="fade-up">
      <div v-if="vpnStore.sessionSeconds >= 5 && vpnStore.status.connected && vpnStore.tunnelBlocked" class="calib-row calib-row--warn">
        <ShieldAlert :size="13" class="calib-row-icon" />
        <span class="calib-row-text" v-text="t('calib.blocked')" />
        <button type="button" class="calib-btn" @click="goServers">
          <span v-text="t('calib.blockedAction')" />
        </button>
      </div>
    </Transition>
    </div>

    <div v-if="isSidebarMode" class="conn-servers" data-sensitive>
      <ConnServerPanel />
    </div>
  </div>
</template>
...
import ConnServerPanel from '../components/ConnServerPanel.vue';
...
const isInlineMode = computed(() => vpnStore.settings.connection_servers === 'inline');
const isSidebarMode = computed(() => vpnStore.settings.connection_servers === 'sidebar');
...
.conn-servers {
  ...
  position: sticky;
  top: 0;
  animation: servers-in 420ms cubic-bezier(0.22, 1, 0.36, 1) both;
}
...
.server-list-reveal-enter-active {
  transition:
    opacity 260ms cubic-bezier(0.22, 1, 0.36, 1),
    transform 300ms cubic-bezier(0.34, 1.3, 0.64, 1);
}
.server-list-reveal-leave-active {
  transition:
    opacity 160ms ease,
    transform 180ms ease;
}
.server-list-reveal-enter-from,
.server-list-reveal-leave-to {
  opacity: 0;
