...
const HOTKEY_ID_PANIC: i32 = 2;

static LOOP_STARTED: AtomicBool = AtomicBool::new(false);
static LOOP_THREAD_ID: AtomicU32 = AtomicU32::new(0);
static PENDING_TOGGLE: Mutex<Option<Option<(u32, u32)>>> = Mutex::new(None);
static PENDING_PANIC: Mutex<Option<Option<(u32, u32)>>> = Mutex::new(None);
static APP_HANDLE: Mutex<Option<AppHandle>> = Mutex::new(None);

pub fn apply(app: &AppHandle, combo: Option<String>, panic_combo: Option<String>) -> Result<(), String> {
    let parsed_toggle = match combo {
        Some(c) if !c.trim().is_empty() => Some(parse_combo(c.trim())?),
        _ => None,
    };
    let parsed_panic = match panic_combo {
        Some(c) if !c.trim().is_empty() => Some(parse_combo(c.trim())?),
        _ => None,
    };
    *APP_HANDLE.lock().unwrap() = Some(app.clone());
    *PENDING_TOGGLE.lock().unwrap() = Some(parsed_toggle);
    *PENDING_PANIC.lock().unwrap() = Some(parsed_panic);
...
fn apply_pending(id: i32, slot: &Mutex<Option<Option<(u32, u32)>>>, registered: &mut bool) {
    ...
    if let Some(next) = next {
        ...
        if let Some((mods, vk)) = next {
            match unsafe {
                RegisterHotKey(HWND(0), id, HOT_KEY_MODIFIERS(mods | MOD_NOREPEAT.0), vk)
            } {
                Ok(()) => *registered = true,
                Err(e) => notify_error(format!("hotkey #{}: {}", id, e)),
            }
        }
    }
}
...
fn hotkey_loop() {
    unsafe {
        ...
        let mut reg_toggle = false;
        let mut reg_panic = false;
        loop {
            apply_pending(HOTKEY_ID_TOGGLE, &PENDING_TOGGLE, &mut reg_toggle);
            apply_pending(HOTKEY_ID_PANIC, &PENDING_PANIC, &mut reg_panic);
...
fn trigger_panic() {
    let app = APP_HANDLE.lock().unwrap().clone();
    if let Some(app) = app {
        let _ = app.emit_all("wawity-hotkey-panic", ());
    }
}
