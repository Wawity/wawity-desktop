...
pub async fn measure_tunnel_latency(
    ...
    tokio::task::spawn_blocking(move || probe_socks(host.as_deref()))
        .await
        .map_err(|e| e.to_string())?
        .map(Some)
}
...
pub async fn set_start_on_boot(enabled: bool) -> Result<(), String> {
    tokio::task::spawn_blocking(move || -> Result<(), String> {
        ...
        if enabled {
            // Scheduled Task refused (AV / policy / disabled Task Scheduler
            // service). Fall back to the HKCU Run key — works everywhere,
            // needs no admin and no scheduler.
            let exe_path = wawity_core::engine::resolve_own_exe_path()?;
            let escaped = exe_path.replace('\'', "''");
            let fallback = format!(
                "$run = 'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Run'; New-Item -Path $run -Force | Out-Null; Set-ItemProperty -Path $run -Name 'Wawity' -Value '\"{0}\" --autostart' -Type String; Write-Output ok",
                escaped
            );
            let fb = crate::util::run_ps_script(&fallback, Duration::from_secs(10))
                .map_err(|e| format!("registry autostart failed: {}", e))?;
            if fb.status.success() {
                crate::util::net_debug_log("autostart: task scheduler refused, fell back to Run key");
                return Ok(());
            }
            let fb_err = String::from_utf8_lossy(&fb.stderr).trim().to_string();
            return Err(format!(
                "failed to update autostart task: {} | registry fallback: {}",
                stderr, fb_err
            ));
        }

        // disabling: also make sure the legacy Run key is gone via reg.exe,
        // which does not depend on PowerShell at all
        let mut reg = crate::util::silent_command("reg");
        reg.args([
            "delete",
        ...
        .stderr(Stdio::null());
        let _ = reg.status();
        Err(format!("failed to update autostart task: {}", stderr))
    }).await.map_err(|e| e.to_string())?
}
